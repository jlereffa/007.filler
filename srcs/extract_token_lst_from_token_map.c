...
/*   Updated: 2017/08/13 11:38:03 by jlereffa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			extract_token_lst_from_token_map(t_filler_var *v)
{
	...
	while (v->token_map[++i])
	{
		...
		{
			if ((v->token_map[i][j] == '.' || v->token_map[i][j] == '*') &&
				is_origin_point && !(is_origin_point = 0))
				set_origin_coordinates(&x_origin, &y_origin, i, j);
			if (v->token_map[i][j] == '*' && !(v->token = set_t_filler_token(
